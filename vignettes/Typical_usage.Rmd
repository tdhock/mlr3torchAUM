...
if(torch_available){
  ...
  for(batching in names(batch_sampler_list)){
    L <- mlr3torch::LearnerTorchMLP$new(task_type="classif")
    my_env <- new.env()
    L$loss <- mlr3torchAUM:::get_nn_ROCAUM_loss(my_env)
    ...
    loss_eval_dt_list[[batching]] <- with(my_env, data.table(
      batching, evals, zeros, all_one_class))
    history_dt_list[[batching]] <- data.table(
      batching,
      L$model$callbacks$history)
    weight_list[[batching]] <- torch::as_array(
      L$model$network$parameters[["0.weight"]]$flatten())
  }
  print(do.call(data.table, weight_list), nrow=10)
}
```
