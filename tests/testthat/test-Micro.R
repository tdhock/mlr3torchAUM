...
test_that("micro_auc: basic properties and small known cases", {
  ...
  ##  Case 1: perfect 3-class separation  micro AUC = 1
  ...
  auc_pkg <- ROC_AUC_micro(pred, labels)$item()
  ...
  auc_pkg2 <- ROC_AUC_micro(probs2, labels2)$item()
...
test_that("micro_aum: basic properties and small known cases", {
  skip_on_cran()
  ...
  aum_pkg <- ROC_AUM_micro(pred, labels)$item()
